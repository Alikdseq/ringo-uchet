...
# Выполнение: powershell -ExecutionPolicy Bypass -File .\scripts\fix-gradle-cache-manual.ps1
...
$gradleProcesses = Get-Process -Name "java" -ErrorAction SilentlyContinue | Where-Object { 
    try {
        $_.CommandLine -like "*gradle*" -or (Get-WmiObject Win32_Process -Filter "ProcessId = $($_.Id)").CommandLine -like "*gradle*"
    } catch {
        $false
    }
}
