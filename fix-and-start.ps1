# Скрипт для исправления и запуска проекта (PowerShell)

Write-Host "🔧 Исправление и запуск Ringo Uchet..." -ForegroundColor Cyan

Write-Host "`n1️⃣ Останавливаем контейнеры..." -ForegroundColor Yellow
docker compose down

Write-Host "`n2️⃣ Пересобираем образы с новыми зависимостями..." -ForegroundColor Yellow
docker compose build --no-cache django-api celery celery-beat

Write-Host "`n3️⃣ Запускаем контейнеры..." -ForegroundColor Yellow
docker compose up -d

Write-Host "`n4️⃣ Ждём инициализации базы данных (10 секунд)..." -ForegroundColor Yellow
Start-Sleep -Seconds 10

Write-Host "`n5️⃣ Применяем миграции..." -ForegroundColor Yellow
docker compose exec django-api python manage.py migrate

Write-Host "`n6️⃣ Проверяем статус WeasyPrint..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "   Затем в Python shell выполните:" -ForegroundColor White
